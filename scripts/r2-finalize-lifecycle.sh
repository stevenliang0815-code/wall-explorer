...
try:
    ...
    open_units=int(con.execute("SELECT count(*) FROM builder_checkpoints WHERE status!='completed'").fetchone()[0])
...
if any((duplicates,open_units,survivor,lookahead)): raise SystemExit("Semantic validation failed")
